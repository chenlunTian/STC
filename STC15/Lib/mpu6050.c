...
float Gyro_x;        //x轴陀螺仪数据暂存					//由角速度计算的倾斜角度
float Angle_ax;      //由加速度计算的倾斜角度 x方向
float Angle_ay;			//由加速度计算的倾斜角度 y方向
...
//	Angle_ax = 57.3*asin(Accel_x/Accel_z);	//计算加速度偏离x方向的角度，asinf(弧度)，两个16384约去了
	Angle_ay = 57.3*asin(Accel_y/Accel_z);	//计算加速度偏离y方向的角度，asinf(弧度)，两个16384约去了
	/*		
...
//	//Angle_gy = Angle_gy + Gyro_y*0.01;  //角速度积分的倾斜角度
	...
	//范围为2000deg/s时，换算关系：16.4 LSB/(deg/s)
	//注意小车前进方向为Y方向
	Gyro_x = (float)GetData(GYRO_XOUT_H);	 //静止时Y轴输出为-30左右
	Gyro_x = -(Gyro_x)/16.4f;         //去除零点便偏移，计算角速度，负号为方向处理
	//Angle_gy = Angle_gy + Gyro_y*0.01;  //角速度积分的倾斜角度
	
	//-------卡尔曼滤波融合-------------------

	Kalman_Filter(Angle_ay,Gyro_x);  //卡尔曼滤波计算倾角，Angle,Angle_ax,Angle_ay,Gyro_x均为全局变量
	printf("Kalman_Filter is ok\n\t");
	//互补滤波
